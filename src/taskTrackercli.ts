...
function loadJSONFile(fileName:string): Data[] {
...
function saveJSONfile(fileName:string, tasks:Data[]){
    const newData = JSON.stringify(tasks)
    fs.writeFileSync(fileName, newData, 'utf-8')
}

function generateUniqueId(tasks: Data[]): number {
    if (tasks.length === 0) {
...
function main() {
    ...
    const task: string|undefined = argv[3];
    ...
    switch (command) {
        ...
        case 'list':
                tasks.forEach(
                    t => console.log(`${t.id} ${t.task} ${t.status} ${t.date}`))
            break;
